...
class ConditionalPriorLoss(_Loss):
    ...
    def forward(self, z_mean: torch.Tensor, z_log_sigma: torch.Tensor, z_wc_mean: torch.Tensor,
        ...
        # z_mean = z_mean.permute(0, 2, 3, 1)
        # z_log_sigma = z_log_sigma.permute(0, 2, 3, 1)
        z_mean = torch.tile(z_mean, [1, self.dim_c, 1, 1])
        z_log_sigma = torch.tile(z_log_sigma, [1, self.dim_c, 1, 1])
        ...
        con_prior_loss = torch.mean(torch.sum(torch.multiply(kl, pc), [1, 2, 3]))
    ...
    def forward(self, pc: torch.Tensor):
        ...
        Args:
            pc: tensor of size (N, C, H, W),
                with N = nr slices, C = dim_z*dim_c, H/W = height/weight of latent space
        """
        closs1 = torch.sum(torch.multiply(pc, torch.log(pc * self.dim_c + 1e-8)), [1])
